classdef EAG_SubFcn
    ...
    methods(Static)
        ...
        function pout=BinomTest(s,n,p,Sided)
            ...
            s=floor(s);
            [s,n,p]= EAG_SubFcn.EqArrayAndScalars(s,n,p);
            E=p.*n;
            GreaterInds=s>=E;
        ...
        function violinplot_advanced(ViolinData, pos, width, properties)
            ...
            properties = EAG_SubFcn.FillEmptyPropertyValues_violin(properties);
            % Exclude NaNs
        ...
        function beeswarmplot_advanced(BeeData, pos, width, properties)
            ...
            properties = EAG_SubFcn.FillEmptySwarmPropertyValues(properties);
            % Exclude NaNs
        ...
        function hpf = tc_highpass(x, tau)
            % TC_HIGHPASS returns the high-pass filtered input signals x, i.e.,
            % the column vectors of x are treated as independent signals.
            % This is achieved by subtracting the low-pass filtered signal from
            % the original signal, thus keeping the high-frequency components.
            % The time constant tau influences the cut-off frequency similar to the
            % low-pass filter case.

            % Preallocation for high-pass filtered signal
            hpf = nan(size(x,1),size(x,2));
            % Preallocation for low-pass filtered signal (temporary)
            lpf = nan(size(x,1),size(x,2));
            % Initialize low-pass filter with the first sample
            lpf(1) = x(1);
            % Iterate over x to create low-pass filtered signal
            for t = 2:length(x)
                % Calculate the change in signal for low-pass filtering
                delta = 1-exp(1)^(-(x(t)-lpf(t-1))/tau);
                lpf(t) = lpf(t-1) + delta;
            end
            % Subtract the low-pass filtered signal from the original signal
            % to obtain the high-pass filtered signal
            hpf = x - lpf;
        end%FCN:tc_highpass
        ...
        function lpf = tc_lowpass(x, tau)
            ...
            % Initialize
            lpf(1) = x(1);
            % Iterate over x and low-pass filter it with a given time constant tau
            for t = 2:length(x)
