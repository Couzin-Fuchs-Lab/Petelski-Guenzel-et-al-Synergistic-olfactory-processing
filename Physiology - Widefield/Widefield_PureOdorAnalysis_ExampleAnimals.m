%% PureOdorAnalysis_ExampleAnimals
% ...
%
% Version:
...
mkdir('PureOdorAnalysis_ExampleAnimals')

%% Settings

% Provide a list of example animals (selected based on BF)
SET.example_animals.gregarious = {...
    '...\Data\Physiology\Cal520_Widefield\COL_HEX3_MOL_N2_OC3L2_ZHAE2\gregarious\220708_Animal066_greg_socialmodulation\'};

SET.example_animals.solitarious = {...
    '...\Data\Physiology\Cal520_Widefield\COL_HEX3_MOL_N2_OC3L2_ZHAE2\solitarious\220509_Animal030_soli_socialmodulation\'};
...
for iPhase = 1:length(SET.phases)
    ...
    for iAni = 1:length(SET.example_animals.(SET.phases{iPhase}))
        ...
        axis equal off
        title('BF')
        export_fig(['PureOdorAnalysis_ExampleAnimals', '\', curr.phase,'_BF'],'-pdf')
        close(hFig_BF)
        ...
        for iStim = 1:length(SET.StimList)
            ...
            print(['PureOdorAnalysis_ExampleAnimals', '\', curr.phase,'_heatmap_', SET.StimList{iStim}],'-dpdf')
            close(hFig_HM)
        ...
        figure(hFig_TC)
        export_fig(['PureOdorAnalysis_ExampleAnimals', '\', curr.phase,'_TC'],'-pdf')
        close(hFig_TC)
