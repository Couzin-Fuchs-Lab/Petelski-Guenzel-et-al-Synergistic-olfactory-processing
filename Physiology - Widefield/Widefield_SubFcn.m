classdef Widefield_SubFcn
    ...
    methods(Static)
        ...
        function pout=BinomTest(s,n,p,Sided)
            ...
            s=floor(s);
            [s,n,p]= Widefield_SubFcn.EqArrayAndScalars(s,n,p);
            E=p.*n;
            GreaterInds=s>=E;
        ...
        function violinplot_advanced(ViolinData, pos, width, properties)
            ...
            properties = Widefield_SubFcn.FillEmptyPropertyValues_violin(properties);
            % Exclude NaNs
        ...
        function beeswarmplot_advanced(BeeData, pos, width, properties)
            ...
            properties = Widefield_SubFcn.FillEmptySwarmPropertyValues(properties);
            % Exclude NaNs
        ...
        function [elbow_x_value, elbow_y_value] = find_elbow_point(x_values, y_values)
            ...
            distances = arrayfun(@(x, y) Widefield_SubFcn.distance_to_line(p1, p2, [x, y]), x_values, y_values);
        ...
        function [bandwidth,density,X,Y]=kde2d(data,n,MIN_XY,MAX_XY)
            ...
            initial_data=Widefield_SubFcn.ndhist(transformed_data,n);
            % discrete cosine transform of initial data
            a= Widefield_SubFcn.dct2d(initial_data);
            % now compute the optimal bandwidth^2
            I=(0:n-1).^2; A2=a.^2;
            t_star=Widefield_SubFcn.root(@(t)(t-Widefield_SubFcn.evolve(t)),N);
            p_02=Widefield_SubFcn.func([0,2],t_star);p_20=Widefield_SubFcn.func([2,0],t_star); p_11=Widefield_SubFcn.func([1,1],t_star);
            ...
            if nargout>1
                density=Widefield_SubFcn.idct2d(a_t)*(numel(a_t)/prod(scaling));
        ...
        function  [out,time]=evolve(t)
            global N
            Sum_func = Widefield_SubFcn.func([0,2],t) + Widefield_SubFcn.func([2,0],t) + 2*Widefield_SubFcn.func([1,1],t);
        ...
        function out=func(s,t)
            global N
            if sum(s)<=4
                Sum_func=Widefield_SubFcn.func([s(1)+1,s(2)],t)+Widefield_SubFcn.func([s(1),s(2)+1],t); const=(1+1/2^(sum(s)+1))/3;
                time=(-2*const*Widefield_SubFcn.K(s(1))*Widefield_SubFcn.K(s(2))/N/Sum_func)^(1/(2+sum(s)));
                out=Widefield_SubFcn.psi(s,time);
            else
                out=Widefield_SubFcn.psi(s,t);
            end
