...
for iAni = 1:size(curr.dir.all,1)
    ...
    if ~isempty(strfind(curr.dir.all(iAni).name, 'Animal')) && curr.dir.all(iAni).isdir
        ...
        % Prepare results
        Segmentation.pocket_active_img = zeros([size(Segmentation.pockets), length(meta.unique_stim)]);
        ...
        for iStim = 1:length(meta.unique_stim)
            ...
            if isfile([curr.path.segmentation, meta.unique_stim{iStim},'.mat'])
                ...
                pool_avg = zeros(length(pocket_list), 1);
                ...
                for iPockte = 1:length(pocket_list)
                    ...
                    pool_tc(iPockte,:) = mean(dat_reshape(idx,:));
                    pool_avg(iPockte,1) = mean(pool_tc(iPockte,meta.activeRegion));
                end%iPocket

                % Get all pockets that are above the threshold
                Segmentation.active_threshold(iStim) = graythresh(pool_avg);
                idx_active = find(pool_avg>Segmentation.active_threshold(iStim));

                % Iterate over all pockets and create a mask
                pocket_active_img = zeros(size(Segmentation.pockets));
