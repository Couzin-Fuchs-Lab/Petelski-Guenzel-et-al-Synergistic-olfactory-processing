...
classdef SubFcn
    ...
    % Version: 13-Dec-2022 (MATLAB R2022a)
    properties
    end
    ...
    methods(Static)
        ...
        function Par = CircleFit(XY)
            ...
            centroid = nanmean(XY);   % the centroid of the data set
        ...
        function violinplot_advanced(ViolinData, pos, width, properties)
            ...
            properties = SubFcn.FillEmptyPropertyValues_violinplot(properties);
            % Exclude NaNs
            data.all = ViolinData(~isnan(ViolinData));
            % Get nanmean and nanmedian
            data.median = nanmedian(data.all );
            data.mean = nanmean(data.all );
        ...
        function out = FillEmptyPropertyValues_violinplot(properties)
            ...
            default.medianSymbol =       'o';                %(Set how the nanmedian should be depicted as symbol (e.g. 'x') or as line: 'line')
            default.medianWidth =        5;                  %(Set width of the nanmedian's line and/or symbol)
            default.meanCol =            'k';                %(Set colour of mean. If it should be depicted)
            default.meanSymbol =         'line';             %(Set how the nanmean should be depicted as symbol (e.g. 'x') or as line: 'line')
            default.meanWidth =          2;                  %(Set width of the nanmedian's line and/or symbol)
        ...
        function boxplot_advanced(data, pos, width, properties)
            ...
            %       properties.medianSymbol =       'line';             %(Set how the nanmedian should be depicted: as a line: 'line', as a symbol: 'x', or both 'line+x')
            %       properties.medianWidth =        2;                  %(Set width of the nanmedian's line and/or symbol)
            %       properties.meanCol =            'w';                %(Set colour of mean. If it should be depicted)
            %       properties.meanSymbol =         'line';             %(Set how the nanmean should be depicted: as a line: 'line', as a symbol: 'x', or both 'line+x')
            %       properties.meanWidth =          2;                  %(Set width of the nanmedian's line and/or symbol)
            %       properties.OutlierCol =         'k';                %(Set the colour in which outliers should be depicted)
            ...
            %   |               |
            %   |               |
            %   |               |           95% CI of the nanmedian (upper margin)
            %    \             /
            %     \           /
            %      \         /
            %       ----o----               nanmedian (depicted as line and symbol
            %      /         \
            %     /           \
            %    /             \
            %   |               |           95% CI of the nanmedian (lower margin)
            %   |               |
            %   |               |
            %   |_______________|           Q1
            %           |
            ...
            properties = SubFcn.FillEmptyPropertyValues_boxplot(properties, width);

            % Tell the user, that if notches are enabled, the line indicating the
            % nanmean's position might exeed the box's edges.
            if (strcmp(properties.Type, 'mean') || strcmp(properties.Type, 'both')) && length(properties.medianSymbol)<=4
                warning('Line indicating position of the data"s nanmean might exceed the box?s edges!')
            end

            % Exclude NaNs
            data(isnan(data)) = [];

            % Calculate values of the boxplot (nanmedian, quantiles, spread, MinValue and MaxValue)
            nan_median = nanmedian(data);
            nan_mean = nanmean(data);
            ...
            % Set length of the nanmedian
            nanmedian_length = [pos-(width/2), pos+(width/2)];
            if strcmp(properties.Notch, 'true')
                ...
                nanmedian_length = [pos-(width/2-width*properties.NotchWidth), pos+(width/2-width*properties.NotchWidth)];
            end%if
            ...
            if strcmp(properties.Orientation, 'vertical')% Plot a vertical boxplot
                ...
                % Plot box
                SubFcn.PlotBox(properties, pos, width, data, Q1, Q3, nan_median, nanmedian_length)

                % Add an option to choose between nanmean, nanmedian or both to plot
                if strcmp(properties.Type, 'median')

                    % - - - Plot nanmedian - - -
                    if strcmp(properties.medianSymbol, 'line')
                        % As a line
                        plot(nanmedian_length, [nan_median, nan_median], 'Color', properties.medianCol, 'LineWidth', properties.medianWidth)
                    elseif length(properties.medianSymbol) >5 && strcmp(properties.medianSymbol(1:5), 'line+')
                        % As line and symbol
                        plot(nanmedian_length, [nan_median, nan_median], 'Color', properties.medianCol, 'LineWidth', properties.medianWidth)
                        plot(pos, nan_median, 'Color', properties.medianCol, 'Marker', properties.medianSymbol(end), 'MarkerFaceColor', properties.medianCol, 'MarkerSize', properties.medianWidth)
                    else
                        % Only as a symbol
                        plot(pos, nan_median, 'Color', properties.medianCol, 'Marker', properties.medianSymbol, 'MarkerFaceColor', properties.medianCol, 'MarkerSize', properties.medianWidth)
                    end
                ...
                elseif strcmp(properties.Type, 'mean') %-------------------------------
                    ...
                    if strcmp(properties.meanSymbol, 'line')
                        % As a line
                        plot([pos-(width/2), pos+(width/2)],[nan_mean, nan_mean], 'Color', properties.meanCol, 'LineWidth', properties.meanWidth)
                    elseif length(properties.meanSymbol) >5 && strcmp(properties.meanSymbol(1:5), 'line+')
                        % As line and symbol
                        plot([pos-(width/2), pos+(width/2)],[nan_mean, nan_mean], 'Color', properties.meanCol, 'LineWidth', properties.meanWidth)
                        plot(pos, nan_mean, 'Color', properties.meanCol, 'Marker', properties.meanSymbol(end), 'MarkerFaceColor', properties.meanCol, 'MarkerSize', properties.meanWidth)
                    else
                        % Only as a symbol
                        plot(pos, nan_mean, 'Color', properties.meanCol, 'Marker', properties.meanSymbol, 'MarkerFaceColor', properties.meanCol, 'MarkerSize', properties.meanWidth)
                    end

                elseif strcmp(properties.Type, 'both') %-------------------------------

                    % First, plot the nanmedian (same code as above for nanmedian only, i.e. same properties
                    if strcmp(properties.medianSymbol, 'line')
                        % As a line
                        plot(nanmedian_length, [nan_median, nan_median], 'Color', properties.medianCol, 'LineWidth', properties.medianWidth)
                    elseif length(properties.medianSymbol) >5 && strcmp(properties.medianSymbol(1:5), 'line+')
                    ...
                    % Second, plot the nanmean (same code as above for nanmean only, i.e. same properties
                    if strcmp(properties.meanSymbol, 'line')
                        % As a line
                        plot([pos-(width/2), pos+(width/2)],[nan_mean, nan_mean], 'Color', properties.meanCol, 'LineWidth', properties.meanWidth)
                    elseif length(properties.meanSymbol) >5 && strcmp(properties.meanSymbol(1:5), 'line+')
            ...
            elseif strcmp(properties.Orientation, 'horizontal')% Plot a horizontal boxplot
                ...
                if strcmp(properties.Type, 'median')
                    ...
                    if strcmp(properties.medianSymbol, 'line')
                        % As a line
                        plot([nan_median, nan_median], nanmedian_length, 'Color', properties.medianCol, 'LineWidth', properties.medianWidth)
                    elseif length(properties.medianSymbol) >5 && strcmp(properties.medianSymbol(1:5), 'line+')
                        % As line and symbol
                        plot([nan_median, nan_median], nanmedian_length, 'Color', properties.medianCol, 'LineWidth', properties.medianWidth)
                        plot(nan_median, pos, 'Color', properties.medianCol, 'Marker', properties.medianSymbol(end), 'MarkerFaceColor', properties.medianCol, 'MarkerSize', properties.medianWidth)
                    else
                        % Only as a symbol
                        plot(nan_median, pos, 'Color', properties.medianCol, 'Marker', properties.medianSymbol, 'MarkerFaceColor', properties.medianCol, 'MarkerSize', properties.medianWidth)
                    end
                ...
                elseif strcmp(properties.Type, 'mean') %-------------------------------
                    ...
                    if strcmp(properties.meanSymbol, 'line')
                        % As a line
                        plot([nan_mean, nan_mean], [pos-(width/2), pos+(width/2)], 'Color', properties.meanCol, 'LineWidth', properties.meanWidth)
                    elseif length(properties.meanSymbol) >5 && strcmp(properties.meanSymbol(1:5), 'line+')
                        % As line and symbol
                        plot([nan_mean, nan_mean], [pos-(width/2), pos+(width/2)], 'Color', properties.meanCol, 'LineWidth', properties.meanWidth)
                        plot(nan_mean, pos, 'Color', properties.meanCol, 'Marker', properties.meanSymbol(end), 'MarkerFaceColor', properties.meanCol, 'MarkerSize', properties.meanWidth)
                    else
                        % Only as a symbol
                        plot(nan_mean, pos, 'Color', properties.meanCol, 'Marker', properties.meanSymbol, 'MarkerFaceColor', properties.meanCol, 'MarkerSize', properties.meanWidth)
                    end
        ...
        end%FCN:boxplot_advanced

        function out = FillEmptyPropertyValues_boxplot(properties, width)
            % FillEmptyPropertyValues(properties) checks whether a property has been
            % set by the user. If not, replace it by its default value. If no property
            % value has been set, completely use defaults.
            ...
            default.medianSymbol =       'line';             %(Set how the nanmedian should be depicted: as a line: 'line', as a symbol: 'x', or both 'line+x')
            default.medianWidth =        2;                  %(Set width of the nanmedian's line and/or symbol)
            default.meanCol =            'w';                %(Set colour of mean. If it should be depicted)
            default.meanSymbol =         'line';             %(Set how the nanmean should be depicted: as a line: 'line', as a symbol: 'x', or both 'line+x')
            default.meanWidth =          2;                  %(Set width of the nanmedian's line and/or symbol)
            default.OutlierCol =         'k';                %(Set the colour in which outliers should be depicted)
        ...
        end%FCN:FillEmptyPropertyValues_boxplot

        function PlotBox(properties, pos, width, data, Q1, Q3, nan_median, nan_median_length)
            % PlotBox(nanmedian, Q1, Q3, properties) plots the box with or without notches
            ...
            elseif strcmp(properties.Notch, 'true')% ----------------------------------
                CI = SubFcn.ConfidenceInterval_nanmedian(data);
                % Vertical, w/ notches ------------------------------------------------
                if strcmp(properties.Orientation, 'vertical')
                    X = [pos-(width/2), pos+(width/2), pos+(width/2), nan_median_length(2), pos+(width/2), pos+(width/2), pos-(width/2), pos-(width/2), nan_median_length(1), pos-(width/2)];
                    Y = [Q3, Q3, CI(2), nan_median, CI(1), Q1, Q1, CI(1), nan_median, CI(2)];
                    h = patch(X, Y, 'k');
                ...
                elseif strcmp(properties.Orientation, 'horizontal')
                    X = [Q1, CI(1), nan_median, CI(2), Q3, Q3, CI(2), nan_median, CI(1), Q1];
                    Y = [pos+(width/2), pos+(width/2), nan_median_length(2), pos+(width/2), pos+(width/2), pos-(width/2), pos-(width/2), nan_median_length(1), pos-(width/2), pos-(width/2)];
                    h = patch(X, Y, 'k');
                    h.FaceColor = properties.BoxFaceCol;
                    h.EdgeColor = properties.BoxEdgeCol;
                    h.LineWidth = properties.BoxEdgeWidth;
        ...
        function CI = ConfidenceInterval_nanmedian(data)
            % Compute the 95% confidence interval (CI) on the nanmedian of a 1D vector
            % Set Z for a 95% CI
            Z = 1.96;
        ...
        end%FCN:ConfidenceInterval_nanmedian
        ...
        function [ActivePixels, ROI] = cropActivePixels(SET, curr, ActivePixels, meta)
            % Check how to crop the active pixels
            if strcmp(SET.crop, 'rectangle')
                % --- Let the user define a rectangle ---
                % Open figure
                hFig = figure;
                % Show image
                img = squeeze(std(curr.refResponse,[],3));
                img = img.*(ActivePixels+0.5);
                imagesc(img);
                % Let the user click on the corners of the rectangle
                title('click on the diagonal corners')
                temp = ginput(2);
                % Get crop region
                crop_x = round(sort(temp(:,1)'),0);
                crop_y = round(sort(temp(:,2)'),0);
                % Check whether it is possible to crop as defined by the
                % user
                if crop_y(2)>size(ActivePixels,1)
                    crop_y(2)=size(ActivePixels,1);
                end
                if crop_x(2)>size(ActivePixels,2)
                    crop_x(2)=size(ActivePixels,2);
                end
                if crop_y(1)<1
                    crop_y(1)=1;
                end
                ...
                % Crop active regions
                ActivePixels(1:crop_y(1),:) = 0;
                ActivePixels(crop_y(2):end,:) = 0;
                ActivePixels(:,1:crop_x(1)) = 0;
                ActivePixels(:,crop_x(2):end) = 0;
                % Get corresponding ROI
                ROI = ones(size(ActivePixels));
                ROI(1:crop_y(1),:) = 0;
                ROI(crop_y(2):end,:) = 0;
                ROI(:,1:crop_x(1)) = 0;
                ROI(:,crop_x(2):end) = 0;
                % Close figure
                close(hFig)
            elseif ~isempty(strfind(SET.crop, 'auto-'))
                % --- Automatically crop based on distribution of active pixels ---
                x = cumsum(sum(ActivePixels,1)/sum(ActivePixels(:)))*100;
                y = cumsum(sum(ActivePixels,2)/sum(ActivePixels(:)))*100;
                region = [(100-str2double(SET.crop(6:end)))/2, str2double(SET.crop(6:end)) + (100-str2double(SET.crop(6:end)))/2];
                [~, crop_x(1)] = min(abs(x-region(1)));
                [~, crop_x(2)] = min(abs(x-region(2)));
                [~, crop_y(1)] = min(abs(y-region(1)));
                [~, crop_y(2)] = min(abs(y-region(2)));
                crop_x = round(crop_x, 0);
                crop_y = round(crop_y, 0);
                % Check whether it is possible to crop as defined by the
                % user
                if crop_y(2)>size(ActivePixels,1)
                    crop_y(2)=size(ActivePixels,1);
                ...
                ROI(:,1:crop_x(1)) = 0;
                ROI(:,crop_x(2):end) = 0;
            elseif ~isempty(strfind(SET.crop, 'density_square-'))
                % --- Automatically define a square around the density maximum of all active pixels ---
                % Get 2D density of pixels
                densActivePixels = imgaussfilt(double(ActivePixels), max(size(ActivePixels))*0.1);
                % Find the maximum
                [max_rows, max_cols] = find(densActivePixels == max(densActivePixels(:)),1);
                % Define region
                val = str2double(SET.crop(16:end));
                crop_x = [max_cols-val, max_cols+val];
                crop_y = [max_rows-val, max_rows+val];
                crop_x = round(crop_x, 0);
                crop_y = round(crop_y, 0);
                % Check whether it is possible to crop as defined by the
                % user
                ...
                ROI(:,1:crop_x(1)) = 0;
                ROI(:,crop_x(2):end) = 0;
            elseif ~isempty(strfind(SET.crop, 'density_circle-'))
                % --- Automatically define a cirle around the density maximum of all active pixels ---
                % Get 2D density of pixels
                densActivePixels = imgaussfilt(double(ActivePixels), max(size(ActivePixels))*0.2);
                % Find the maximum
                [~,max_rows] = max(sum(densActivePixels,1));
                [~,max_cols] = max(sum(densActivePixels,2));
                % Define radius
                val = str2double(SET.crop(16:end));
                % Find outlying pixels
                [idx_rows, idx_cols] = find(ActivePixels>-inf);
                idx_rows = idx_rows-max_rows;
                idx_cols = idx_cols-max_cols;
                dist2max = sqrt(sum([idx_rows, idx_cols]'.*[idx_rows, idx_cols]'))';
                % kick them out
                ActivePixels(dist2max>val) = 0;
                % Get corresponding ROI
                ROI = ones(size(ActivePixels));
                ROI(dist2max>val) = 0;
            elseif ~isempty(strfind(SET.crop, 'point2_circle'))
                % --- Let the user define a circular ROI ---
                % Show image
                hFig = figure('units', 'normalized', 'position', [0 0 1 1]);
                imagesc(squeeze(std(curr.refResponse,[],3)))
                axis equal off
                title('click 4 times')
                % Get ROI and indicate selection
                Par = SubFcn.CircleFit(fliplr(ginput(4)));
                hold on
                th = 0:pi/50:2*pi;
                plot(Par(3) * cos(th) + Par(2),  Par(3) * sin(th) + Par(1), 'r')
                pause(1)
                % Find outlying pixels
                max_rows = Par(1);
                max_cols = Par(2);
                ...
                % Close figure
                close(hFig)
            elseif ~isempty(strfind(SET.crop, 'load_circle'))
                % --- Load a previously defined circular ROI ---
                max_rows = meta.circleROI_par(1);
                max_cols = meta.circleROI_par(2);
                val = meta.circleROI_par(3);
                [idx_rows, idx_cols] = find(ActivePixels>-inf);
                idx_rows = idx_rows-max_rows;
                idx_cols = idx_cols-max_cols;
                dist2max = sqrt(sum([idx_rows, idx_cols]'.*[idx_rows, idx_cols]'))';
                ...
                ROI = ones(size(ActivePixels));
                ROI(dist2max>val) = 0;
            elseif isempty(SET.crop)
                ROI = ones(size(ActivePixels));
            end%if
            ActivePixels = ActivePixels==1;
            ROI = ROI==1;
        end%FCN:cropActivePixels
        ...
        function stack_out = poolFilter(stack_in, SET)
            % poolFilter(stack_in, SET) applies a non-overlapping 2D kernel
            % to shrink the input stack_in.
            % First, let us map which input pixels correspong to which
            % output pixel
            targetMask = 1:(SET.TargetSize(1)*SET.TargetSize(2));
            targetMask = reshape(targetMask, SET.TargetSize);
            poolMask = imresize(targetMask, size(stack_in,[1 2]),'nearest');
            % Next, preallocation of the output
            stack_out = zeros([SET.TargetSize,size(stack_in,3)]);
            % Check which method to apply. Do this before the for-loop to
            % save some time
            switch SET.poolMethod
                case 'min_pool'
                    % Iterate over all pixels and pool by taking the
                    % current quadrant's minimum value
                    for iPx = 1:SET.TargetSize(1)*SET.TargetSize(2)
                        [R_idx, C_idx] = find(poolMask == targetMask(iPx));
                        [R_idx2, C_idx2] = find(targetMask == targetMask(iPx));
                        stack_out(R_idx2, C_idx2,:) = min(min(stack_in(R_idx, C_idx,:)));
                    end%iPX
                case 'max_pool'
                    % Iterate over all pixels and pool by taking the
                    % current quadrant's maximum value
                    for iPx = 1:SET.TargetSize(1)*SET.TargetSize(2)
                        [R_idx, C_idx] = find(poolMask == targetMask(iPx));
                        [R_idx2, C_idx2] = find(targetMask == targetMask(iPx));
                        stack_out(R_idx2, C_idx2,:) = max(max(stack_in(R_idx, C_idx,:)));
                    end%iPX
                case 'mean_pool'
                    % Iterate over all pixels and pool by taking the
                    % current quadrant's nanmean value
                    for iPx = 1:SET.TargetSize(1)*SET.TargetSize(2)
                        [R_idx, C_idx] = find(poolMask == targetMask(iPx));
                        [R_idx2, C_idx2] = find(targetMask == targetMask(iPx));
                        stack_out(R_idx2, C_idx2,:) = nanmean(nanmean(stack_in(R_idx, C_idx,:)));
                    end%iPX
                case 'median_pool'
                    % Iterate over all pixels and pool by taking the
                    % current quadrant's nanmedian value
                    for iPx = 1:SET.TargetSize(1)*SET.TargetSize(2)
                        [R_idx, C_idx] = find(poolMask == targetMask(iPx));
                        [R_idx2, C_idx2] = find(targetMask == targetMask(iPx));
                        stack_out(R_idx2, C_idx2,:) = nanmedian(nanmedian(stack_in(R_idx, C_idx,:)));
                    end%iPX
            end%switch
        end%FCN:poolFilter

        function [pockets_bw, pockets, pockets_labeled, pocket_Zscore, pocket_Zscore_img, pocket_Corr, pocket_Corr_img] = stackSegmentation(stack, stack_info, img_mask, regmax_method, n_rep, minPixCount, Zscore_props)
            % stackSegmentation(stack, n_rep) segments the spatial aspects
            % of a 3-dimensionalstack (x*y*time) into individual regions
            % based on a refined 2D Delaunay triangulation. Regions are
            % divided into two classes (active, not active), based on
            % z-scores.
            %
            % Input:
            %       stack         : 3-D matrix (x*y*time)
            %       stack_info    : 1-D matrix (time*1)
            %       img_mask      : 2-D matrix (x*y) of ones and zeros
            %                       indicating valid pixel
            %       regmax_method : methof for determining how to identify
            %                       local maxima
            %                       - 'raw'      - simply apply imregionalmax on the std-projection
            %                       - 'filtered' - apply imregionalmax on movmax-filtered std-projection
            %                       - 'both'     - combine both above-mentioned methods
            %       n_rep         : Number of iterations for refining the
            %                       regions.
            %       Zscore_props  : struct containing the three fields
            %                       - Baseline [Default: all frames]
            %                          Array of frames indices used to
            %                          detemine baseline activity.
            %                       - Window [Default: all frames]
            %                          Array of frames indices used to
            %                          determine the period of
            %                          stimulus-induced activity.
            %                       - active_Zscore [Default: 3.5]
            %                          minimum z-score an area has to cross
            %                          in order to be  counted as an active
            %                          one for a givenstimulus.
            % Output:
            %       pockets_bw         : binary image showing all valid areas
            %       pockets            : grayscale showing pocket distribution
            %       pockets_labeled    : pocket labels
            %       pocket_Zscore      : z-score of each pocket for each stimulus (n_pockets*stim)
            %       pocket_Zscore_img  : z-score images (x*y*stim)
            %       pocket_Corr        : within-pocket correlation for each stimulus (n_pockets*stim)
            %       pocket_Corr_img    : correlation images (x*y*stim)
            %
            % Version: 05-Dec-22 (R2022a)


            % Replace not given property values by defaults
            if nargin < 7
                Zscore_props.Baseline = 1:length(find(stack_info == stack_info(1)));
                Zscore_props.Window = 1:length(find(stack_info == stack_info(1)));
                Zscore_props.active_Zscore = 3.5;
            end
            ...
            % Get std-projection from stack
            BF_std = nanstd(stack, [], 3);
            % Correct outliers.
            % --- Get min and max
            Q1 = quantile(BF_std(:), 0.25);
            Q3 = quantile(BF_std(:),0.75);
            Spread = 1.5*(Q3-Q1);
            MaxValue = Q3 + Spread;
            MaxValue = quantile(BF_std(:), 1-(length(find(BF_std>MaxValue))/numel(BF_std)));
            MinValue = Q1 - Spread;
            MinValue = quantile(BF_std(:), length(find(BF_std<MinValue))/numel(BF_std));
            % --- apply threshold
            BF_std(BF_std<MinValue) = MinValue;
            BF_std(BF_std>MaxValue) = MaxValue;
            clear Q1 Q3 Spread MaxValue MinValue


            % Substitute NaN with the minimum value
            BF_std(isnan(BF_std)) = min(BF_std(:));
    

            % Identify local maxima
            BF_std = imgaussfilt(BF_std,1);
            switch regmax_method
                case 'raw'
                    regmax_img = imregionalmax(BF_std,8)>0;
                    regmin_img = imregionalmin(BF_std,8)>0;
                case 'filtered'
                    % Filter to combine pockets that belong to the same large scale
                    % maximum/minimum
                    BF_std_max_filtered = movmax(movmax(BF_std',3)',3);
                    regmax_img = imregionalmax(BF_std_max_filtered,8)>0;
                    BF_std_min_filtered = movmin(movmin(BF_std',3)',3);
                    regmin_img = imregionalmin(BF_std_min_filtered,8)>0;
                case 'both'
                    % Filter to combine pockets that belong to the same large scale
                    % maximumminimum
                    BF_std_max_filtered = movmax(movmax(BF_std',3)',3);
                    regmax_img = (imregionalmax(BF_std_max_filtered,8) + imregionalmax(BF_std,8))>0;
                    BF_std_min_filtered = movmin(movmin(BF_std',3)',3);
                    regmin_img = (imregionalmin(BF_std_min_filtered,8) + imregionalmin(BF_std,8))>0;
            end
            originalBW = (regmax_img+regmin_img)>0;
            ...
            dt = delaunayTriangulation([locMax.x(:), locMax.y(:)]);
            [voronoi_V,voronoi_R] = voronoiDiagram(dt);
            VoroDiag = [locMax.x(:), locMax.y(:)];
            % --- Identify pockets and label them
            pockets_labeled = zeros(size(BF_std));
            cnt = 1;
            for iPocket = 1:size(VoroDiag,1)
                A=voronoi_V(voronoi_R{iPocket},:);
                B=A(any(~isinf(A),2),:);
                if(size(B,1)>2)
                    idx_map = poly2mask(B(:,1), B(:,2), size(BF_std,1), size(BF_std,2));
                    pockets_labeled(find(idx_map)) =  cnt;
                    cnt = cnt+1;
                end%if
            end%iPocket
            pockets_labeled = pockets_labeled.*img_mask;
            clear A B dt voronoi_* cnt iPocket

            % Nice, before we start the segmentation! Let us go over all
            % pockets and sort them
            % -------------------------------------------------------------
            pocketList = unique(pockets_labeled);
            pocketIdx = nan(length(pocketList),2);
            for iPocket = 1:length(pocketList)
                if pocketList(iPocket)~=0
                    pocketIdx(iPocket,1) = pocketList(iPocket);
                    pocketIdx(iPocket,2) = find(pockets_labeled==pocketList(iPocket), 1, 'first');
                else
                    pocketIdx(iPocket,1:2) = 0;
                end%if zero
            end%iPocket
            [~,I] = sort(pocketIdx(:,2));
            temp = zeros(size(pockets_labeled));
            for iPocket = 1:length(pocketList)
                temp(pockets_labeled == pocketList(I(iPocket))) = iPocket-1;
            end%iPocket
            ...
            for iRep = 1:n_rep
                ...
                pocketTC = nan(size(VoroDiag,1), size(stack,3));
                pocketList = unique(pockets_labeled);
                for iPocket = 1:length(pocketList)
                ...
                idx_candidates_newIdentiy = SubFcn.refinement_parfor(idx_candidates, idx_table, pockets_labeled, reshaped_stack, pocketTC, pocketList, nMask);
                            ...
                            for iPx = 1:length(idx_candidates)
                                ...
                                if ~isempty(nC)
                                    ...
                                    for iN = 1:length(nC)
                                        ...
                                        r2 = corrcoef(activity_px, activity_cluster).^2; voronoi_R(iN) = r2(2);
            ...
            % Nice, the segmentation is done now! Let us go over all
            % pockets again and sort them
            % -------------------------------------------------------------
            pocketList = unique(pockets_labeled);
            pocketIdx = nan(length(pocketList),2);
            for iPocket = 1:length(pocketList)
            ...
            [~,I] = sort(pocketIdx(:,2));
            temp = zeros(size(pockets_labeled));
            pockets = zeros(size(pockets_labeled));
            for iPocket = 1:length(pocketList)
                temp(pockets_labeled == pocketList(I(iPocket))) = iPocket-1;
                pockets(pockets_labeled == pocketList(I(iPocket))) = rand(1);
            end%iPocket
            pockets_labeled = temp;
            pockets = pockets.*img_mask;
            pockets_labeled = pockets_labeled.*img_mask;


            % Get each pocket's zScore for each stimulus and the
            % within-pocket avg correlation among pixles
            % -------------------------------------------------------------
            pocketList = unique(pockets_labeled);
            pocketList = pocketList(pocketList~=0);
            stimList = unique(stack_info);
            pocket_Zscore = nan(length(pocketList), length(stimList));
            pocket_Corr = nan(length(pocketList), length(stimList));
            for iPocket = 1:length(pocketList)
                idx_pocket = find(pockets_labeled==pocketList(iPocket));
                for iStim = 1:length(stimList)
                    idx_stim = find(stack_info == stimList(iStim));
                    % Get within-pocket correlation
                    r = tril(corrcoef(reshaped_stack(idx_pocket, idx_stim)'), -1);
                    mask = tril(ones(size(r)), -1);
                    pocket_Corr(iPocket,iStim) = mean(r(mask==1));
                    % Each pocket's zScore
                    if length(idx_pocket)>1
                        pocketActivity = nanmean(reshaped_stack(idx_pocket, idx_stim));
                    else
                        pocketActivity = reshaped_stack(idx_pocket, idx_stim);
                    end
                    mu = nanmean(pocketActivity(Zscore_props.Baseline));
                    s = nanstd(pocketActivity(Zscore_props.Baseline));
                    z = (pocketActivity - mu)/s;
                    pocket_Zscore(iPocket,iStim) = nanmean(z(Zscore_props.Window));
                end%iStim
                clear idx_pocket idx_stim pocketActivity overallActivity r
            end%iPocket
            clear x mu s r mask
            % Turn this into images for each stimulus
            pocket_Zscore_img = zeros([size(pockets), length(stimList)]);
            pocket_Corr_img = zeros([size(pockets), length(stimList)]);
            for iStim = 1:length(stimList)
                for iPocket = 1:length(pocketList)
                    temp = zeros(size(pockets));
                    idx = find(pockets_labeled==pocketList(iPocket));
                    if pocketList(iPocket) == -1
                        temp(idx) = 0;
                        pocket_Zscore_img(:,:,iStim) = pocket_Zscore_img(:,:,iStim)+temp;
                        pocket_Corr_img(:,:,iStim) = pocket_Zscore_img(:,:,iStim)+temp;
                    else
                        temp(idx) = pocket_Zscore(iPocket,iStim);
                        pocket_Zscore_img(:,:,iStim) = pocket_Zscore_img(:,:,iStim)+temp;
                        temp(idx) = pocket_Corr(iPocket,iStim);
                        pocket_Corr_img(:,:,iStim) = pocket_Corr_img(:,:,iStim)+temp;
                    end
                    clear temp idx
                end%iPocket
            end%iStim
            ...
            % Merge pockets that are not activity-related
            % -------------------------------------------------------------
            % Kick out everything that is not contribution much
            pockets_bw = imfill(max(pocket_Zscore_img,[],3)>Zscore_props.active_Zscore ,"holes");
        ...
        function [Stack, SET, curr] = LoadStack(SET, curr, iTrial)
            switch SET.DataFormat
                case 'tiffs'
                    ...
                    protocol = readtable([curr.path.trial,'\protocol.txt'], opts);
                    % Itentify sequence of stimuli
                    curr.stimuli_seq{iTrial} = split(protocol.Value(find(strcmp(protocol.Parameter, 'Stimuli'))), ', ');
                    % Get duration of each stimulus [frames]
                    curr.duration{iTrial} = str2double(protocol.Value(find(strcmp(protocol.Parameter, 'Duration'))));
                    % Get stimulus onset
                    curr.protocol{iTrial}.StimON = [];
                    curr.protocol{iTrial}.StimON = ones(length(curr.stimuli_seq{iTrial}),1)*str2double(protocol.Value(find(strcmp(protocol.Parameter, 'S1_on'))));
                    % Get basename
                    SET.filetype = '.tiff';
                    content = dir(curr.path.trial);
                    filenames = {content(:).name}';
                    filenames = filenames(~cellfun(@isempty, strfind(filenames, SET.filetype)));
                    SET.basename = filenames{1};
                    while true
                        if sum(~cellfun(@isempty, strfind(filenames, SET.basename))) == length(filenames)
                            break
                        else
                            SET.basename = SET.basename(1:end-1);
                        end%if
                    end%while
                    % Identify prefix
                    SET.prefix = [];
                    ...
                    % stack
                    for iFrame = 1:(length(curr.stimuli_seq{iTrial})*curr.duration{iTrial})
                        ...
                        % Resize snap
                        if ~isempty(SET.TargetSize)
                            snap = imresize(snap, SET.TargetSize, 'box');
                        else
                            SET.TargetSize = size(snap);
                        end
                        % Stack
                        Stack(:,:,iFrame) = snap;
                    end%iFrame
                ...
                case 'lsm'
                    ...
                    curr.protocol{iTrial}.StimON = ones(length(curr.stimuli_seq{iTrial}),1)*str2double(protocol.Value(find(strcmp(protocol.Parameter, 'S1_on'))));
                    % Get list of files
                    curr.lsmdir = dir(curr.path.trial);
                    ...
                    for iStim = 1:length(curr.stimuli_seq{iTrial})
                        ...
                        % of [time, x, y]
                        idx = find(~cellfun(@isempty, strfind({curr.lsmdir(:).name}, ['_',curr.stimuli_seq{iTrial}{iStim},'_'])));
                        if ~isempty(idx)
                ...
                case 'pst'
                    ...
                    % Get duration of each stimulus [frames]
                    curr.duration{iTrial} = curr.protocol{iTrial}.num_frames(1);
                    % Get image sizes
                    img_size = [curr.protocol{iTrial}.img_width(1), curr.protocol{iTrial}.img_height(1)];
                    % Iterate over all pst files and create a stack
                    Stack = [];
                    for iStim = 1:length(curr.stimuli_seq{iTrial})
                        % Get stacks of current stimulus
                        fr340 = SubFcn.pstread(img_size(1), img_size(2), curr.duration{iTrial}, [curr.path.trial, '\', curr.protocol{iTrial}.DBB1{iStim}, '.pst']);
                        fr380 = SubFcn.pstread(img_size(1), img_size(2), curr.duration{iTrial}, [curr.path.trial, '\', curr.protocol{iTrial}.dbb2{iStim}, '.pst']);
                        % Iterate over frames, smooth and form the ratio
                        for iFrame = 1:curr.duration{iTrial}
